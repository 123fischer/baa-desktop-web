  ...
  if (auctions.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-neutral-tint">No auctions match your filters.</p>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="divide-y divide-neutral-200">
        {auctions.map((auction) => (
          <div
            key={auction.id}
            className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_60px] gap-4 items-center px-4 py-4 text-sm hover:bg-neutral-shade"
          >
            <Image
              src={auction.image}
            ...
            >
              {auction.isFavorite ? (
                ...
                <StarOutlineIcon className="w-5 text-neutral-tint" />
              )}
            </button>
          </div>
        ))}
